    ...
    for l in range(4):
        ...
        for k in range(4):
            ...
            second_dihedral = H_lattice.set_dihedral(
                int(indices[1]), oxygens[1][k], silicons[1][k], hydrogen[1], 180, mask=None
            )
            H_lattice.translate(-1 * diff)
            H_lattice.wrap()
            ...
            if path:
                os.makedirs(
                    "{0}/D-{1}-{2}".format(path, labels[oxygens[0][l]], labels[oxygens[1][k]]),
                    exist_ok=True,
                )

                write(
                    "{0}/D-{1}-{2}/POSCAR".format(
                        path, labels[oxygens[0][l]], labels[oxygens[1][k]]
                    ),
                    H_lattice,
                    sort=False,
                )
